USE [master]
GO
IF EXISTS (SELECT name FROM master.dbo.sysdatabases WHERE name = N'PE_PRN_Fall22B1')
BEGIN
	ALTER DATABASE [PE_PRN_Fall22B1] SET OFFLINE WITH ROLLBACK IMMEDIATE;
	ALTER DATABASE [PE_PRN_Fall22B1] SET ONLINE;
	DROP DATABASE [PE_PRN_Fall22B1];
END
